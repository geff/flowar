...
namespace NewFlowar
{
    public class GameFlowar : GameBase
    {
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            //if (Contexte.ContextType == ContextType.CardRotated ||
            //    Contexte.ContextType == ContextType.CardSelected)
            //{
            //    Contexte.ScaleSelectedCard = Contexte.lerpScale.Eval(gameTime);
            //}

            ////---> Animation de rotation de la carte
            //if (Contexte.ContextType == Contexte.ContextType.CardRotated)
            //{
            //    Contexte.CurrentCardRotation = Contexte.lerpRotation.Eval(gameTime);

            //    if (Contexte.lerpRotation.IsFinished(gameTime))
            //        Contexte.ContextType = Contexte.ContextType.CardSelected;
            //}

            ////---> Animation de rotation de la carte sur la map
            //if (Contexte.ContextType == ContextType.CardRotatedOverMap)
            //{
            //    Contexte.CurrentCardRotation = Contexte.lerpRotation.Eval(gameTime);

            //    if (Contexte.lerpRotation.IsFinished(gameTime))
            //    {
            //        Contexte.ContextType = ContextType.CardOverMap;
            //        //TODO : voir si cette ligne de code est utile par la suite
            //        Contexte.oldCloneSelectedCard = Contexte.CloneSelectedCard;
            //    }
            //}
        ...
        private void StartGame()
        {
            ...
            for (int i = 1; i <= Contexte.ListAllPlayerColor.Keys.Count; i++)
            {
                DistributeCards(i);
            }
        ...
        {
            Contexte.ListModelCard = new List<ModelCard>();

            Contexte.ListModelCard.Add(new ModelCard(2043));
            Contexte.ListModelCard.Add(new ModelCard(50103));
            Contexte.ListModelCard.Add(new ModelCard(6023));
            Contexte.ListModelCard.Add(new ModelCard(1024));
            Contexte.ListModelCard.Add(new ModelCard(4012));
            Contexte.ListModelCard.Add(new ModelCard(4013));

            foreach (ModelCard card in Contexte.ListModelCard)
            {
                //TODO:
